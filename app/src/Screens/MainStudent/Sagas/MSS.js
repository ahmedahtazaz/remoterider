...
function* loadAdmobId(action) {

    let id = yield* loadAdmobIdInner();

    if(id)
    {
        yield put({type: LOAD_ADMOB_ID_SUCCESS, admobId: id});
    }
    else
        yield put({type: LOAD_ADMOB_ID_FAILURE});
}

function* loadAdmobIdInner() {

    let id = undefined;

    yield firestore().collection('Admob').doc('Admob').get().
    then((doc) => {
        if(doc && doc.data())
        {
            id = doc.data().AdId;
...
export default function* loadDataActionWatcher() {
    ...
    yield takeLatest(`${LOAD_ADMOB_ID}`, loadAdmobId);
}
